...
class WaveNetBlock(nn.Module):
    def __init__(self, channels: int, dilation: int, skip_channels: int):
        super().__init__()
        padding = dilation
        self.filter_conv = nn.Conv1d(channels, channels, kernel_size=2, dilation=dilation, padding=padding)
        self.gate_conv = nn.Conv1d(channels, channels, kernel_size=2, dilation=dilation, padding=padding)
...
class WaveNet(nn.Module):
    def __init__(self, in_channels: int, residual_channels: int = 32, skip_channels: int = 64, dilations=None):
        super().__init__()
...
def train_wavenet(model: WaveNet, loader: DataLoader, epochs: int = 20, lr: float = 1e-3, checkpoint: str = "./services/checkpoints/wavenet.pth") -> WaveNet:
...
def load_wavenet(input_dim: int, checkpoint: str = "./services/checkpoints/wavenet.pth") -> WaveNet:
